...
const MLPrediction = () => {
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: 2 }, bgcolor: bgMain, minHeight: '100vh', color: textPrimary }}>
      {/* Header */}
      <Card elevation={4} sx={{ borderRadius: 2, bgcolor: cardBg, mb: 2 }}>
        <CardContent sx={{ p: { xs: 2, sm: 3 }, py: 2 }}>
          <Box display="flex" alignItems="center" justifyContent="space-between">
            <Box display="flex" alignItems="center" gap={1.5}>
              <AutoGraphIcon sx={{ fontSize: 32, color: '#6870fa' }} />
      ...
      <Grid container spacing={1.5} sx={{ mb: 2 }}>
        <Grid item xs={4}>
          <Card sx={{ bgcolor: cardBg, borderRadius: 2, height: '100%' }}>
            <CardContent sx={{ p: 1.5, textAlign: 'center' }}>
              <AttachMoneyIcon sx={{ fontSize: 28, color: '#4caf50', mb: 0.5 }} />
        ...
        <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          <Box sx={{ flex: 1, overflowY: 'auto', p: 2, bgcolor: bgMain }}>
            {messages.map((msg, i) => (
              <Box key={i} sx={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start', mb: 2 }}>
                <Box sx={{ maxWidth: '85%', display: 'flex', gap: 1, flexDirection: msg.role === 'user' ? 'row-reverse' : 'row' }}>
                  <Box sx={{ width: 32, height: 32, borderRadius: '50%', bgcolor: msg.role === 'user' ? userBubble : '#4caf50', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {msg.role === 'user' ? <PersonIcon sx={{ fontSize: 18, color: '#fff' }} /> : <SmartToyIcon sx={{ fontSize: 18, color: '#fff' }} />}
                  </Box>
            ...
            {chatLoading && (
              <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                ...
                <Paper elevation={2} sx={{ p: 1.5, bgcolor: assistantBubble, borderRadius: 2 }}>
                  <Typography variant="body2" color="#212121">Đang suy nghĩ...</Typography>
                </Paper>
              </Box>
            )}
            <div ref={messagesEndRef} />
